...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"flage\":\"%d\",", Data_init.Device_State); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"window\":\"%d\",", device_state_init.check_device); 
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp\":\"%.2f\",", Data_init.temperatuer);
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi\":\"%.2f\"", Data_init.humiditr); 
	strcat(buf, text);
